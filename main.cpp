...
void packet_handler(unsigned char* user, const struct pcap_pkthdr* pkthdr, const unsigned char* packet) {
    ...
    // Important: The physicall layer is alredy unpacking the frame form layer 1, basically preamble and sfd is already cut off when we receive it here
    ...
    if (ether_type == 0x0806) {  // ARP EtherType
        const unsigned char* arp_packet = packet + 14; // ARP packet starts at offset 14
        ...
        uint16_t protocol_type = (arp_packet[2] << 8) | arp_packet[3];  // This is mostly ipv4 0x0800. 
                                                                        // It identifies the network layer protocol for which arp is resolving the address
        uint8_t hardware_size = arp_packet[4]; // Length of hardware length (Mac address)
        uint8_t protocol_size = arp_packet[5]; // Length of protocol network address ipv4 = 4 byte example: 255.255.255.255 
        uint16_t operation_code = (arp_packet[6] << 8) | arp_packet[7]; // Specifies the operation that the sender is performing: 1 for request, 2 for reply.

        std::cout << "ARP Frame Information:\n";
        std::cout << "Hardware Type: 0x" << std::hex << hardware_type << std::dec << "\n";
        ...
        for (int i = 0; i < 4; ++i) {
            std::cout << static_cast<int>(arp_packet[SENDER_PROTOCOL_ADDRESS + i]);
            if (i < 3) std::cout << ".";
        }
        std::cout << "\n";
        ...
        for (int i = 0; i < 4; ++i) {
            std::cout << static_cast<int>(arp_packet[TARGET_PROTOCOL_ADDRESS + i]);
            if (i < 3) std::cout << ".";
        }
        std::cout << "\n";
    }
